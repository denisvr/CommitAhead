# CommitAhead - reproducible local dev database bootstrap: roles -> migrations -> RLS(users) -> RLS(Phase 1) -> RLS(Phase 2) -> RLS(Phase 3) -> RLS(Phase 4).
#
...
# 004_rls_phase2.sql/005_rls_phase3.sql/007_rls_phase4.sql all drop+recreate their policies instead
# of failing on a second run.
#
...
try {
    ...
    Get-Content "scripts/database/007_rls_phase4.sql" -Raw | docker compose exec -T db psql -v ON_ERROR_STOP=1 -U postgres -d commitahead
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
