...
public sealed class RlsIsolationPhase2Tests : IAsyncLifetime
{
    ...
    // CA2100 cannot be satisfied here and must not be. These are the repository's own DDL scripts
    // (backend/scripts/database/*.sql), read from disk and executed verbatim to build the RLS test
    // schema; PostgreSQL does not accept parameters for role names or role passwords, so CREATE/ALTER
    // ROLE cannot be parameterized. The only substituted values are the two container-only passwords
    // generated by this fixture, which never leave the disposable test database. No caller passes
    // external input, and the production application never runs these scripts.
    [SuppressMessage("Security", "CA2100:Review SQL queries for security vulnerabilities", Justification = "Repository-owned DDL bootstrap scripts; PostgreSQL cannot parameterize role DDL. No external input reaches this method.")]
