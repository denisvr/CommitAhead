...
public class ReplaceCertificationsUseCaseTests
{
    ...
    public async Task ExecuteAsync_WithAnExistingProfile_ReplacesCertificationsAndReturnsSuccess()
    {
        ...
        var useCase = new ReplaceCertificationsUseCase(repository, new StubCurrentUser { UserId = ownerUserId, Email = "owner@example.com" });
    ...
    {
        var repository = new FakeProfessionalProfileRepository();
        var useCase = new ReplaceCertificationsUseCase(repository, new StubCurrentUser { UserId = Guid.NewGuid(), Email = "owner@example.com" });
