...
public class ReplaceSkillsUseCaseTests
{
    ...
    public async Task ExecuteAsync_WithAnExistingProfile_ReplacesSkillsAndReturnsSuccess()
    {
        ...
        var useCase = new ReplaceSkillsUseCase(repository, new FakeCVPresentationRepository(), new StubCurrentUser { UserId = ownerUserId, Email = "owner@example.com" });
    ...
    {
        var repository = new FakeProfessionalProfileRepository();
        var useCase = new ReplaceSkillsUseCase(repository, new FakeCVPresentationRepository(), new StubCurrentUser { UserId = Guid.NewGuid(), Email = "owner@example.com" });
