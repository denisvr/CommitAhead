...
public class ReplaceEducationUseCaseTests
{
    ...
    public async Task ExecuteAsync_WithAnExistingProfile_ReplacesEducationAndReturnsSuccess()
    {
        ...
        var useCase = new ReplaceEducationUseCase(repository, new StubCurrentUser { UserId = ownerUserId, Email = "owner@example.com" });
    ...
    {
        var repository = new FakeProfessionalProfileRepository();
        var useCase = new ReplaceEducationUseCase(repository, new StubCurrentUser { UserId = Guid.NewGuid(), Email = "owner@example.com" });
