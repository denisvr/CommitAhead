...
public class ReplaceCertificationSelectionsUseCaseTests
{
    ...
    public async Task ExecuteAsync_SelectingAnExistingEntry_ReplacesSelectionsAndReturnsSuccess()
    {
        ...
        profile.ReplaceCertifications([entry], DateTime.UtcNow);
        ...
        var useCase = new ReplaceCertificationSelectionsUseCase(cvRepository, profileRepository, new StubCurrentUser { UserId = ownerUserId, Email = "owner@example.com" });
        ...
        Assert.Equal([entry.Id], presentation.CertificationSelections);
    }
    ...
    public async Task ExecuteAsync_WithNoExistingPresentation_ReturnsNotFound()
    {
        var cvRepository = new FakeCVPresentationRepository();
        var profileRepository = new FakeProfessionalProfileRepository();
        var useCase = new ReplaceCertificationSelectionsUseCase(cvRepository, profileRepository, new StubCurrentUser { UserId = Guid.NewGuid(), Email = "owner@example.com" });
