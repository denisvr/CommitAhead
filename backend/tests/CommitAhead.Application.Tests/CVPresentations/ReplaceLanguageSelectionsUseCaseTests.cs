...
public class ReplaceLanguageSelectionsUseCaseTests
{
    ...
    public async Task ExecuteAsync_SelectingAnExistingEntry_ReplacesSelectionsAndReturnsSuccess()
    {
        ...
        profile.ReplaceLanguages([entry], DateTime.UtcNow);
        ...
        var useCase = new ReplaceLanguageSelectionsUseCase(cvRepository, profileRepository, new StubCurrentUser { UserId = ownerUserId, Email = "owner@example.com" });
        ...
        Assert.Equal([entry.Id], presentation.LanguageSelections);
    }
    ...
    public async Task ExecuteAsync_WithNoExistingPresentation_ReturnsNotFound()
    {
        var cvRepository = new FakeCVPresentationRepository();
        var profileRepository = new FakeProfessionalProfileRepository();
        var useCase = new ReplaceLanguageSelectionsUseCase(cvRepository, profileRepository, new StubCurrentUser { UserId = Guid.NewGuid(), Email = "owner@example.com" });
