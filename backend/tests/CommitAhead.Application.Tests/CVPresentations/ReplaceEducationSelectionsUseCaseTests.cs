...
public class ReplaceEducationSelectionsUseCaseTests
{
    ...
    public async Task ExecuteAsync_SelectingAnExistingEntry_ReplacesSelectionsAndReturnsSuccess()
    {
        ...
        profile.ReplaceEducation([entry], DateTime.UtcNow);
        ...
        var useCase = new ReplaceEducationSelectionsUseCase(cvRepository, profileRepository, new StubCurrentUser { UserId = ownerUserId, Email = "owner@example.com" });
        ...
        Assert.Equal([entry.Id], presentation.EducationSelections);
    }
    ...
    public async Task ExecuteAsync_WithNoExistingPresentation_ReturnsNotFound()
    {
        var cvRepository = new FakeCVPresentationRepository();
        var profileRepository = new FakeProfessionalProfileRepository();
        var useCase = new ReplaceEducationSelectionsUseCase(cvRepository, profileRepository, new StubCurrentUser { UserId = Guid.NewGuid(), Email = "owner@example.com" });
