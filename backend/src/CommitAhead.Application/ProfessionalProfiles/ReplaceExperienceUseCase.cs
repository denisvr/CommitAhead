...
public sealed class ReplaceExperienceUseCase
{
    ...
    public ReplaceExperienceUseCase(IProfessionalProfileRepository repository, ICVPresentationRepository cvPresentationRepository, ICurrentUser currentUser)
    {
    ...
    public async Task<ProfessionalProfileMutationResult> ExecuteAsync(IEnumerable<ExperienceEntry> experience, CancellationToken cancellationToken)
    {
        ...
        var newEntries = experience.ToList();
        var removedIds = profile.Experience.Select(entry => entry.Id).Except(newEntries.Select(entry => entry.Id)).ToHashSet();

        profile.ReplaceExperience(newEntries, DateTime.UtcNow);

        await DanglingSelectionCleanup.RemoveDanglingSelectionsAsync(
            _cvPresentationRepository,
            _currentUser.UserId,
            removedIds,
            presentation => presentation.ExperienceSelections,
            (presentation, ids, updatedAtUtc) => presentation.ReplaceExperienceSelections(ids, updatedAtUtc),
            cancellationToken);
