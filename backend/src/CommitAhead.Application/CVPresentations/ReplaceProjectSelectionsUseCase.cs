...
public sealed class ReplaceProjectSelectionsUseCase
{
    private readonly ICVPresentationRepository _repository;
    private readonly IProfessionalProfileRepository _professionalProfileRepository;
    private readonly ICurrentUser _currentUser;

    public ReplaceProjectSelectionsUseCase(ICVPresentationRepository repository, IProfessionalProfileRepository professionalProfileRepository, ICurrentUser currentUser)
    {
    ...
    public async Task<CVPresentationMutationResult> ExecuteAsync(Guid id, IEnumerable<Guid> entryIds, CancellationToken cancellationToken)
    {
        ...
        var validEntryIds = profile?.Projects.Select(entry => entry.Id).ToHashSet() ?? [];
        var candidateIds = entryIds.ToList();
        if (candidateIds.Any(entryId => !validEntryIds.Contains(entryId)))
        {
            throw new DomainValidationException("entryIds references a Project entry that does not exist in the referenced ProfessionalProfile.");
        }

        presentation.ReplaceProjectSelections(candidateIds, DateTime.UtcNow);
