...
public sealed class ReplaceSkillSelectionsUseCase
{
    private readonly ICVPresentationRepository _repository;
    private readonly IProfessionalProfileRepository _professionalProfileRepository;
    private readonly ICurrentUser _currentUser;

    public ReplaceSkillSelectionsUseCase(ICVPresentationRepository repository, IProfessionalProfileRepository professionalProfileRepository, ICurrentUser currentUser)
    {
    ...
    public async Task<CVPresentationMutationResult> ExecuteAsync(Guid id, IEnumerable<Guid> entryIds, CancellationToken cancellationToken)
    {
        ...
        var validEntryIds = profile?.Skills.Select(entry => entry.Id).ToHashSet() ?? [];
        var candidateIds = entryIds.ToList();
        if (candidateIds.Any(entryId => !validEntryIds.Contains(entryId)))
        {
            throw new DomainValidationException("entryIds references a Skill that does not exist in the referenced ProfessionalProfile.");
        }

        presentation.ReplaceSkillSelections(candidateIds, DateTime.UtcNow);
