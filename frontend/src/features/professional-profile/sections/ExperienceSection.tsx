...
export function ExperienceSection({ experience, skills, onChange, focusRequest, onHighlightAchievement }: ExperienceSectionProps) {
  const { error, handleSave } = useSectionSave(experience, replaceExperience)
  ...
  const dragReorder = useDragReorder(experience, onChange, (next) => void handleSave(next))
              ...
              >
                {sortMode === 'manual' && (
                  <div className={styles.moveRow}>
                    <Button type="button" variant="ghost" onClick={() => moveTo(entry.id, -1)} disabled={index === 0} aria-label={`Move ${entry.role || 'entry'} up`}>
                      Move up
                    </Button>
                    <Button
                      type="button"
