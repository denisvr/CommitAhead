...
`20260820112709_AddProfessionalProfileEntryPositions` itself only added the column with a schema
default of `0` — it ran no backfill statement, so every pre-existing row across all four tables
collapsed to the same position (0) within its own profile, an intra-profile tie the read-side
`OrderBy` could not break deterministically. `20260820144624_BackfillProfessionalProfileEntryPositions`
corrects this: for each (profile, table) group that is both multi-row and still entirely at
position 0, it assigns distinct positions via `ROW_NUMBER() OVER (PARTITION BY
professional_profile_id ORDER BY ctid) - 1` — Postgres's own physical row order, since no other
column records the order those rows were originally entered in. This is a best-effort repair, not
a recovery of the user's actual original intent (never persisted), and it is guarded to skip any
group that already has a real, non-uniform position from an actual `Replace*` save.
