...
> Item 4 of this decision is superseded by ADR-0015: authorization no longer checks `sub` against a single hardcoded `OWNER_USER_ID`. See ADR-0015 for the multi-user-ready owner-scoped model.

## Context

Supabase is typically integrated with a client-side SDK using the public anon key embedded in the frontend. This is appropriate for multi-tenant apps where users own their own data. CommitAhead holds sensitive career data for a small, invite-only set of users and has no public registration flow.

## Decision

Authentication is fully backend-mediated using Supabase magic link with PKCE:

1. Magic-link initiation accepts only pre-provisioned (invited) email addresses, returns a generic response regardless of outcome, and is rate-limited.
...
4. Every API request is validated by middleware: issuer, audience, signature, and expiry are checked against Supabase's JWKS; `sub` must correspond to an existing, enabled application `User` record (see ADR-0015). Any unrecognised Supabase identity receives 403.
...
- Public signup must be disabled in Supabase; every account must be pre-created/invited out-of-band (see ADR-0015).
