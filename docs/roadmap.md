...
- [ ] *(Deferred to Phase 6/deployment, not blocking)* Apply the migration bundle and `001_roles.sql`/`002_rls_users.sql`/`003_rls_phase1.sql` to the real Supabase Postgres, and seed each enabled user's `users` row there — user-run, needs the real DB password
