...
- [ ] Add PDF fixtures/failure tests, source-deletion integration tests, and API/component coverage — PDF fixtures/failure tests (`PdfPigTextExtractorTests`: Malformed/ImageOnly/TooManyPages/TooMuchText against the real extractor), source-deletion integration tests, and API coverage (`JobAnalysesEndpointTests` upload round-trip) exist. Not covered: a deterministic real-PdfPig `TimedOut` fixture (impractical without a pathological file — covered only at the Application-test level via a fake extractor) and a real encrypted-PDF fixture (impractical to hand-craft; the `Encrypted` mapping itself is exercised by `PdfPigTextExtractor`'s own catch clause, just not fixture-driven). Frontend component coverage remains, tracked with the frontend UI above
