...
- [x] Implement AnalyzeJobAnalysis — `AnalyzeJobAnalysisUseCase` (`backend/src/CommitAhead.Application/AI/`): owner-scoped idempotency/concurrency (ADR-0015), the stale-reservation reconciliation above, strict validation of every AI-proposed field (unknown JSON properties and non-exact enum names rejected outright, every referenced Id checked against the catalogue actually sent), a same-response requirement/gap reference mechanism for `AddJobRequirement`/`AddJobGap` that lets one analysis pass propose both together without ever trusting an AI-generated Guid as a real database Id, and an atomic draft-persistence-plus-usage-completion transaction via a new minimal `IUnitOfWork` (reused for the reservation-reconciliation step too). `AnalyzeCVPresentation`/`AnalyzeInterviewNote` are not yet implemented — same shape, later slice
