...
- [x] Implement AIUsageRecord Reserved → Completed/Failed lifecycle and durable idempotency — Domain aggregate (Slice 1) plus EF persistence (Slice 3): `AIUsageRecordRepository`, a real unique database constraint on `idempotency_key` (verified by a test bypassing the application-level check), and `GetSpentCostAsync` for the budget check. Lazy stale-reservation reconciliation is not implemented — it belongs to the reservation transaction inside the (not yet built) AnalyzeX use cases, not the repository itself
