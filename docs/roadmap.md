...
- [x] Wire the first HTTP surface for AnalyzeX/ApplyAnalysisDraft — `POST /api/{job-analyses,cv-presentations,interview-notes}/{id}/analyze` and `POST /api/analysis-drafts/{id}/apply` (new `AnalysisDraftsController`), with a stable machine-readable `outcomeCode` in `ProblemDetails.Extensions` for every non-2xx AI-command response. `ValidationExceptionFilter` (renamed from `DomainValidationExceptionFilter`) now also maps `ApplyAnalysisDraftValidationException` to 422. `GET /api/analysis-drafts/{id}` (reading a draft's proposals for review) is deliberately not built here — it belongs with the draft-review UI below
...
- [ ] Build draft review UI with editable final accepted payloads (needs `GET /api/analysis-drafts/{id}`, not yet built — see above)
- [x] Add adapter tests with stubbed HTTP (`AnthropicAIProviderTests`, `AnthropicHttpClientLoggingTests`), DI-selection tests, budget/idempotency/rate-limit tests, and API-level scenario tests — real-provider integration atomicity was already proven in the previous slice (`ApplyAnalysisDraftIntegrationTests`); no frontend exists yet to test
