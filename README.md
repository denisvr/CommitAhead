...
Phase 5 (CV Export, ADR-0020: PDF via QuestPDF) is in progress — backend only so far, no frontend UI yet. `ExportCVPresentationUseCase` (`GET /api/cv-presentations/{id}/export`) resolves a CVPresentation's selected/ordered ProfessionalProfile entries, applies its `IncludeEmail`/`IncludePhone`/`IncludeAddress` visibility flags (`IncludePhoto` is accepted but not yet actionable — no photo upload/storage path exists anywhere in this codebase), formats `YearMonth` dates locale-aware, and renders the result via `QuestPdfCVExportRenderer` (`IExportRenderer`, Infrastructure) — one A4 template covering every section. `PageLimit` is enforced as a hard cap by reading the rendered PDF back with PdfPig and counting pages, not by constraining layout mid-render. Markdown fields (summary, entry descriptions) go through a new `RestrictedMarkdownParser` (Markdig-based) that mirrors `RestrictedMarkdown.tsx`/`restrictedUrlTransform.ts`'s exact allowlist — no images, no raw HTML, links kept only for https/http/mailto — fully covered by tests. The use case, the renderer's own parsed-output assertions, the endpoint, and the frontend download button are not yet tested/built — see `docs/roadmap.md`'s Phase 5 checklist for the precise breakdown of what's done versus pending.
