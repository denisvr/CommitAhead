...
Phase 5 (CV Export, ADR-0020: PDF via QuestPDF) is done and tested on the backend — the frontend download UI is the only piece not yet built. `ExportCVPresentationUseCase` (`GET /api/cv-presentations/{id}/export`) resolves a CVPresentation's selected/ordered ProfessionalProfile entries, applies its `IncludeEmail`/`IncludePhone`/`IncludeAddress` visibility flags (`IncludePhoto` is accepted but not yet actionable — no photo upload/storage path exists anywhere in this codebase), formats `YearMonth` dates locale-aware, and renders the result via `QuestPdfCVExportRenderer` (`IExportRenderer`, Infrastructure) — one A4 template covering every section. `PageLimit` is enforced as a hard cap by reading the rendered PDF back with PdfPig and counting pages, not by constraining layout mid-render. Markdown fields (summary, entry descriptions) go through `RestrictedMarkdownParser` (Markdig-based), mirroring `RestrictedMarkdown.tsx`/`restrictedUrlTransform.ts`'s exact allowlist — no images, no raw HTML, links kept only for https/http/mailto. All of it — the parser, the use case's selection/visibility/page-limit logic, the renderer's actual PDF output, and the endpoint's HTTP round trip — is covered by tests that assert on real parsed values, never a snapshot. No frontend download button, OpenAPI client regeneration, MSW handler, or component test exists yet — see `docs/roadmap.md`'s Phase 5 checklist for the precise per-item breakdown.
