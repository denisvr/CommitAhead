...
// client-side state). Assertions target the current Home/Sidebar/AccountMenu shell — the
// Professional Profile & CV Presentations product, not the removed Study Queue.
test('unauthenticated visitor is kept out; a minted session authorizes access; logout ends it', async ({ page, authenticatedPage }) => {
  ...
  await test.step('a test-issued session is consumed by the real authentication pipeline and the app shell loads', async () => {
    ...
    await expect(authenticatedPage.getByRole('navigation', { name: 'Primary' })).toBeVisible();
    await expect(authenticatedPage.getByRole('button', { name: 'Professional profile', exact: true })).toBeVisible();
    await expect(authenticatedPage.getByRole('button', { name: 'CV presentations', exact: true })).toBeVisible();
  });
  ...
  await test.step('the account menu shows the signed-in email', async () => {
    await authenticatedPage.getByRole('button', { name: 'Account menu' }).click();
  ...
  await test.step('logout ends the session and protected content is no longer accessible', async () => {
    ...
    await expect(authenticatedPage.getByRole('heading', { name: 'Welcome back' })).not.toBeVisible();
